...
static inline Vector3 quat_differentiate_angular_velocity(Quaternion next, Quaternion curr, real_t dt,
                                                          real_t eps = 1e-8) {
    return quat_to_scaled_angle_axis(quat_abs(next * curr.inverse()), eps) / dt;
}

static void _spring_damper_exact(real_t& x, real_t& v, real_t x_goal, real_t v_goal, real_t damping_ratio,
                                 real_t halflife, real_t dt, real_t eps = 1e-5) {
    real_t g = x_goal;
    real_t q = v_goal;
...
static void _critical_spring_damper_exact(real_t& x, real_t& v, real_t x_goal, real_t v_goal, real_t halflife,
                                          real_t dt) {
    real_t g = x_goal;
    real_t q = v_goal;
    real_t d = halflife_to_damping(halflife);
...
static inline PackedFloat32Array critical_spring_damper_exact(real_t x, real_t v, real_t x_goal, real_t v_goal,
...
static void _simple_spring_damper_exact(Vector3& x, Vector3& v, const Vector3 x_goal, const real_t halflife,
                                        const real_t dt) {
    real_t y = halflife_to_damping(halflife) / 2.0;
    Vector3 j0 = x - x_goal;
    Vector3 j1 = v + j0 * y;
...
static void _simple_spring_damper_exact(Quaternion& x, Vector3& v, const Quaternion x_goal, const real_t halflife,
                                        const real_t dt) {
    real_t y = halflife_to_damping(halflife) / 2.0;
...
static void _timed_spring_damper_exact(real_t& x, real_t& v, real_t& xi, const real_t x_goal, const real_t t_goal,
                                       const real_t halflife, const real_t& dt, real_t apprehension = 2.0) {
...
static inline PackedFloat32Array timed_spring_damper_exact(real_t x, real_t v, real_t xi, const real_t x_goal,
                                                           const real_t t_goal, const real_t halflife, const real_t dt,
...
static Dictionary character_update(Vector3 pos, Vector3 vel, Vector3 acc, Quaternion quaternion,
                                   Vector3 angular_velocity, Vector3 v_goal, Quaternion q_goal, real_t halflife_vel,
    ...
    Dictionary answer;
    {
        ...
        answer["world_position"] =
            eydt * ((-j1 / (y * y)) + ((-j0 - j1 * dt) / y)) + (j1 / (y * y)) + j0 / y + v_goal * dt + pos;
...
static Dictionary character_predict(Vector3 x, Vector3 v, Vector3 a, Quaternion q, Vector3 angular_v, Vector3 v_goal,
                                    Quaternion q_goal, real_t halflife_v, real_t halflife_q,
...
static inline void inertialize_transition(Vector3& off_x, Vector3& off_v, const Vector3 src_x, const Vector3 src_v,
                                          const Vector3 dst_x, const Vector3 dst_v) {
    off_x = (src_x + off_x) - dst_x;
    off_v = (src_v + off_v) - dst_v;
}

static inline void inertialize_update(Vector3& out_x, Vector3& out_v, Vector3& off_x, Vector3& off_v,
                                      const Vector3 in_x, const Vector3 in_v, const real_t halflife, const real_t dt) {
    Spring::_decay_spring_damper_exact(off_x, off_v, halflife, dt);
    out_x = in_x + off_x;
    out_v = in_v + off_v;
}

static inline void inertialize_transition(Quaternion& off_x, Vector3& off_v, const Quaternion src_x,
                                          const Vector3 src_v, const Quaternion dst_x, const Vector3 dst_v) {
    off_x = Spring::quat_abs((off_x * src_x) * dst_x.inverse()).normalized();
    off_v = (off_v + src_v) - dst_v;
}
static inline void inertialize_update(Quaternion& out_x, Vector3& out_v, Quaternion& off_x, Vector3& off_v,
                                      const Quaternion in_x, const Vector3 in_v, const real_t halflife,
                                      const real_t dt) {
    Spring::_decay_spring_damper_exact(off_x, off_v, halflife, dt);
...
static inline Quaternion calculate_offset_quat(const Quaternion src_q, const Quaternion dst_q,
                                               const Quaternion off_q = Quaternion()) {
    return Spring::quat_abs((off_q * src_q) * dst_q.inverse());
}

static inline Dictionary binded_inertia_transition(const Vector3 off_x, const Vector3 off_v, const Vector3 src_x,
