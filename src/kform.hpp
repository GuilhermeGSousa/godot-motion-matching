...
struct kform {
    ...
    kform(Transform3D tr)
        : pos{tr.origin}, rot{tr.basis.get_rotation_quaternion()},
          scl{tr.basis.get_scale()}, vel{}, ang{}, svl{} {
    }

    kform(Vector3 p, Quaternion r, Vector3 s, Vector3 lv, Vector3 av,
          Vector3 sv)
        : pos{p}, rot{r}, scl{s}, vel{lv}, ang{av}, svl{sv} {
    }

    kform(Ref<SkeletonProfile> skel, NodePath bonepath, Ref<Animation> anim,
          double time)
        : kform{skel->get_reference_pose(
              skel->find_bone(bonepath.get_concatenated_subnames()))} {
        auto tpos =
            anim->find_track(bonepath, Animation::TrackType::TYPE_POSITION_3D);
        auto trot =
            anim->find_track(bonepath, Animation::TrackType::TYPE_ROTATION_3D);
        auto tscl =
            anim->find_track(bonepath, Animation::TrackType::TYPE_SCALE_3D);
        kform s1 = *this;
        if (tpos != -1) {
    ...
    enum Space { Local, Model, RootMotion, Global };

    kform(Ref<SkeletonProfile> skel, Ref<Animation> anim, double time,
          NodePath bonepath, Space space)
        : kform(skel->get_reference_pose(
              skel->find_bone(bonepath.get_concatenated_subnames()))) {
        switch (space) {
        case Local:
    ...
    // DONE
    void _local(Ref<SkeletonProfile> skel, Ref<Animation> anim, double time,
                NodePath bonepath) {
        *this = skel->get_reference_pose(
            skel->find_bone(bonepath.get_concatenated_subnames()));
        auto tpos =
            anim->find_track(bonepath, Animation::TrackType::TYPE_POSITION_3D);
        auto trot =
    ...
    // Done
    void _model(Ref<SkeletonProfile> skel, Ref<Animation> anim, double time,
                NodePath bonepath) {
        _global(skel, anim, time, bonepath);
        auto root = kform{};
        root._local(skel, anim, time,
                    bonepath.get_concatenated_names() + String(":") +
                        skel->get_root_bone());
        *this = *this / root;
    }
    // TODO
    void _root(Ref<SkeletonProfile> skel, Ref<Animation> anim, double time,
               NodePath bonepath) {
        _global(skel, anim, time, bonepath);
        auto root = kform{};
        root._local(skel, anim, time,
    ...
    // DONE
    void _global(Ref<SkeletonProfile> skel, Ref<Animation> anim, double time,
                 NodePath bonepath) {
        kform s1 = *this;
        ...
        do {
            kform l = *this;
            l._local(skel, anim, time,
                     skelpath + ':' + UtilityFunctions::str(bone));
            locals.push_back(l);
        ...
        *this = std::accumulate(
            locals.rbegin(), locals.rend(), kform{},
            [](const kform &acc, const kform &i) { return acc * i; });
    }
    ...
    kform &finite_difference(kform input_next, real_t _dt) {
        ...
        ang = Spring::quat_to_scaled_angle_axis(
                  Spring::quat_abs(input_next.rot * rot.inverse())) /
            _dt;
    ...
    static kform finite_difference(const kform &input_curr,
                                   const kform &input_next, real_t _dt) {
        kform out = input_curr;
        out.vel = (input_next.pos - out.pos) / _dt;

        out.ang = Spring::quat_to_scaled_angle_axis(
                      Spring::quat_abs(input_next.rot * out.rot.inverse())) /
            _dt;
    ...
    friend kform operator/(const kform v, const kform w) {
        ...
        out.scl = w.scl / v.scl;
        out.vel = v.rot.xform_inv(w.vel - v.vel -
                                  v.ang.cross(v.rot.xform(out.pos * v.scl))) -
...
struct kforms {
    ...
    kforms(std::size_t N)
        : pos(N, Vector3()), rot(N, Quaternion()), scl(N, Vector3(1, 1, 1)),
          vel(N, Vector3()), ang(N, Vector3()), svl(N, Vector3()) {
    }
